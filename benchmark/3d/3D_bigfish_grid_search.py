"""3D grid search for Big-FISH benchmark."""
...
## Copied RS-FISH benchmark code to get all possible thresholds
...
def _get_spot_counts(thresholds, value_spots):
    ...
    count_spots = np.array([np.log(np.count_nonzero(value_spots > t)) if np.count_nonzero(value_spots > t) 
                            > 0 else 0 for t in thresholds])
...
#path = '/path/to/input/'
path = 'ufish/data_3d/train/'
ims_path = 'image/'

# path = '/path/to/output/
csv_path = 'ufish/benchmarks/3d/BF/results/'
...
def process_im(im, sigma, voxel_size_z, voxel_size_yx, psf_z, psf_yx, im_path, default_sig_yx, gamma_range, alpha_range, beta_range):
    ...
    for threshold in thrs_to_use:
        ...
        if (thr_n_spots not in n_spots):
            ...
            conditions_str = (f'BF_{os.path.basename(im_path)}'
                          f'_sigz{sigma[0]}sigyx{sigma[1]}_thr{threshold:.3f}')

            ## save the spots (int)
            df = pd.DataFrame(data=spots, columns=["axis-0", "axis-1", "axis-2"])
            df_path = os.path.join(os.path.dirname(csv_path),
                                    (f'{conditions_str}_spots-threshold.csv'))
                ...
                for beta in beta_range:
                    ...
                    try:
                        ...
                        if spots_post_decomposition.shape[0] < thr_n_spots:
                            ...
                            df = pd.DataFrame(data=spots_post_decomposition, columns=["axis-0", "axis-1", "axis-2"])
                            df_path = os.path.join(os.path.dirname(csv_path),
                                                   (f'{conditions_str}_alpha{alpha}_beta{beta}_gamma{gamma}_decompose-dense.csv'))
                            df.to_csv(df_path, index=False)
                            print(f'saving to {df_path}')
                    except Exception as e:
    ...
    for sig_z_delta in sigma_z_range:
        ...
        for sig_yx_delta in sigma_yx_range:
            ...
            process_im(im, sigma, voxel_size_z, voxel_size_yx, psf_z, psf_yx,
                    im_path, default_sig_yx, gamma_range, alpha_range, beta_range)
